...
class Mississippi extends Job implements SelfHandling, ShouldQueue
{
    ...
    {
        $this->url_base = 'http://state.sor.dps.ms.gov/';
    ...
    {
        $county = MsCounty::where('status', '=', '0')->orderByRaw("RAND()")->first();
    ...
    private function get_results($county)
    {
        ...
        $county = MsCounty::firstOrNew(['county_id' => $county]);
    ...
    {
        $county = MsCounty::where('status', '=', '1')->orderBy('county_id')->first();
        if ($county) {
            if (App::runningInConsole()) {
            ...
            try {
                $table = $link_crawler->filter('h4')->nextAll();
                $links = $table->filter('a')->extract(array('href'));
                foreach ($links as $link) {
                    $url = str_replace("./", $this->url_base, $link);
                    $hash = md5($url);

                    $profile = MsProfile::firstOrNew(array('hash' => $hash));
                    $profile->url = $url;
                    $profile->hash = $hash;
            ...
            $county = MsCounty::firstOrNew(['county_id' => $curr_county_id]);
            $county->status = $status;
            $county->save();

            $this->state->records_expected = MsProfile::count();
            $this->state->save();
            //
            unset($html);
            unset($link_crawler);
    ...
    private function process_profiles()
    {
        ...
        $profile = MsProfile::where('status', '=', '0')->orderByRaw("RAND()")->first();
        // profile time calcs
        ...
        if ($profile) {
            ...
            $county = MsCounty::where('county_id', '=', $county)->first();
            //
    ...
    private function extract_data($url, $county_name)
    {
        try {
            ...
            echo "**** Request Timed Out - sLeEpInG for 10 seconds";
            sleep(10);
            $this->process_profiles();
        }
        if (!$this->check_uri()) {
        ...
        if (empty($results)) {
            $profile = MsProfile::firstOrNew(['url' => $url]);
            $profile->status = '2';
            $profile->save();
            unset($profile);
            $this->process_profiles();
        ...
        $offences = $this->crawler->filter('#ctl00_ContentPlaceHolder1_OffenderDetails1_offenses tr')->each(function ($node, $i) {
            $date = $node->filter('td')->eq(0)->extract('_text');
            $offence = $node->filter('td')->eq(3)->extract('_text');
            if (!empty($offence[0])) {
        ...
        $offender = MsOffender::firstOrNew(['so_offenderid' => $so_offenderid]);
        ...
        foreach ($offences as $offence) {
            ...
            $offense = MsOffense::firstOrNew(['hash' => $of_hash]);
